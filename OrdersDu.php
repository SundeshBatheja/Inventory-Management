...
$stm->execute();
$query = "DELETE from OrderDetails where Oid = $del[1];";
$stm=$con->prepare($query);
$stm->execute();
...
<?php 
    ...
    foreach($data as $res){
      ...
      <div class = "form-group">
        ...
        <div class = "form-group">
            <label for = "user">Order Date</label> 
            <input type="text" name="date"  value = <?php echo $res['date'];?> id = "user" class = "form-control">
              </div><br>
        <div class = "form-group">
...
$stm->execute();
ob_end_clean();
header("location:uploadOrders.php?msg=success");
};?>